...
class Salvar:
    ...
    def salvar_jogo(self, jogador, inimigo, inimigo_explosivo, mapa):
        ...
        with open(self.__nome_arquivo, 'w') as arquivo:
            json.dump(dados_jogo, arquivo, indent=4)
    ...
    def carregar_jogo(self, jogador, inimigo, inimigo_explosivo, mapa):
        try:
            ...
            mapa.poderes.empty()
            for dados_poder in dados_jogo.get('poderes', []):
                posicao = (dados_poder["x"], dados_poder["y"])
                tipo = dados_poder.get('tipo', 'desconhecido')  # Valor padrão se 'tipo' não estiver presente
                
                # Assumindo tamanho padrão para os poderes
                tamanho = (40, 40)
                
                poder = Poder(posicao=posicao, tamanho=tamanho, tipo=tipo)
